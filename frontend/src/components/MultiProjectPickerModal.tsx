...
export const MultiProjectPickerModal = ({ visible, onClose, onConfirm, selectedProjectIds }: Props) => {
  ...
  useEffect(() => {
    if (visible) {
      fetchProjects();
      setSearchText('');
      setSelectedIds(new Set(selectedProjectIds));
    }
  }, [visible]);
  ...
  const areAllFilteredSelected = filtered.length > 0 && filtered.every(p => selectedIds.has(p._id));
  ...
  const toggleSelectAll = () => {
    if (areAllFilteredSelected) {
      ...
      // Select all currently filtered projects
      setSelectedIds(prev => {
        const next = new Set(prev);
        ...
        <View style={styles.card}>
          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {loading ? (
              ...
              filtered.map(proj => {
                const isSelected = selectedIds.has(proj._id);
                return (
                  <TouchableOpacity
                    key={proj._id}
                    style={[styles.item, isSelected && styles.itemSelected]}
                    onPress={() => toggleProject(proj._id)}
                  >
